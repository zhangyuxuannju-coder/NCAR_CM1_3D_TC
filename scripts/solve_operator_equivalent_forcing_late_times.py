...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--forcing-root",
        default=None,
        help="Root containing tXXXh/operator_perturbation_products.npz for a custom case.",
    )
    p.add_argument("--case-label", default=None, help="Label used for a custom forcing set.")
    p.add_argument(
        "--hours",
        type=float,
        nargs="+",
        default=None,
        help="Hours to solve for a custom forcing set (requires --forcing-root and --case-label).",
    )
    return p.parse_args()


def experiment_specs(args: argparse.Namespace) -> list[dict]:
    custom = (args.forcing_root, args.case_label, args.hours)
    if any(value is not None for value in custom):
        if not all(value is not None for value in custom):
            raise ValueError("--forcing-root, --case-label, and --hours must be supplied together")
        root = Path(args.forcing_root)
        return [
            {
                "case": args.case_label,
                "hour": float(hour),
                "folder": root / f"t{hour:03.0f}h",
            }
