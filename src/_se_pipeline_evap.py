"""
Sawyer-Eliassen Equation Diagnostic Pipeline

运行示例 (Example Command):
python se_diagnostic_pipeline.py --target-time-hours 48 --output-dir se_pipeline_output_full_48h_stable --sor-omega 1.8
"""
...
class PipelineConfig:
    ...
    evap_cooling_q0: float = -2.0e-4     # 峰值冷却率 (K/s), 负值为冷却, -2e-4≈-17K/day
    evap_r_center: float = 145.0          # 椭圆中心半径 (km)
    evap_z_center: float = 15.0           # 椭圆中心高度 (km)
    evap_r_half: float = 105.0            # 径向半轴 (km), 覆盖 40-250km
    evap_z_half: float = 2.5              # 垂直半轴 (km), 覆盖 12.5-17.5km
    evap_dipole: bool = False             # True=加热-冷却偶极子, False=纯冷却高斯
    evap_dipole_q_factor: float = 1.0     # 偶极子模式Q0缩放因子 (峰值≈|Q0|*0.1/σz)
    evap_dipole_sigma_z: float = 1.5      # 偶极子模式专用垂直半轴 (km), 更窄的加热冷却层
...
def azimuthal_average_from_3d(cfg: PipelineConfig) -> Dict[str, np.ndarray]:
    ...
    try:
        ...
        def load_3d(var_name: str, t_idx: Optional[int] = None) -> np.ndarray:
            use_t = time_idx if t_idx is None else int(t_idx)
            data, dims = _get_time_slice(ds[var_name], use_t)
        ...
        v = load_3d(var_map["v"])
        w = load_3d(var_map["w"])
        prs = load_3d(var_map["prs"])
        rho = load_3d(var_map["rho"])
        theta = load_3d(var_map["theta"])

        if var_map["psfc"]:
            psfc_data, psfc_dims = _get_time_slice(ds[var_map["psfc"]], time_idx)
            psfc_s, psfc_dims_s = _destagger_to_scalar_grid(psfc_data, psfc_dims)
            if tuple(psfc_dims_s) != ("yh", "xh"):
                raise ValueError(f"变量 {var_map['psfc']} 维度为 {tuple(psfc_dims_s)}，期望 ('yh','xh')")
        ...
        ur_avg = _azimuthal_average_by_radius(ur_3d, bin_index, valid_mask, nr)
        ut_avg = _azimuthal_average_by_radius(ut_3d, bin_index, valid_mask, nr)
        w_avg = _azimuthal_average_by_radius(w, bin_index, valid_mask, nr)
        prs_avg = _azimuthal_average_by_radius(prs, bin_index, valid_mask, nr)
        ...
        dtheta_dt_avg = np.nan_to_num(dtheta_dt_avg)
        eddy_th_adv_avg = np.nan_to_num(eddy_th_adv_avg)
        V_mzeta = np.nan_to_num(V_mzeta)
        V_ezeta = np.nan_to_num(V_ezeta)
        V_ev = np.nan_to_num(V_ev)
        V_dh = np.nan_to_num(V_dh)
        V_dv = np.nan_to_num(V_dv)
        tramp = np.nan_to_num(tramp)

        return {
            "z_km": z_sel,
            "r_km": r_centers,
            "center_x_km": np.array([xc_km], dtype=np.float64),
            "center_y_km": np.array([yc_km], dtype=np.float64),
            "ur": ur_avg,
            "ut": ut_avg,
            "w": w_avg,
            "prs": prs_avg,
            ...
            "time_seconds_used": np.array([time_selected_sec], dtype=np.float64),
...
def _stratiform_evap_cooling(
    ...
    两种模式:
    - 纯冷却高斯 (evap_dipole=False): 单一椭圆冷却区
    - 加热-冷却偶极子 (evap_dipole=True): 高斯垂直导数
        上部 (z>zc): 凝华/凝结加热 (Q>0)
        下部 (z<zc): 降水蒸发/升华冷却 (Q<0)
    """
    if not cfg.evap_cooling_enabled:
        return np.zeros((len(z_km), len(r_km)), dtype=np.float64)

    R, Z = np.meshgrid(r_km, z_km)
    dist_sq = ((R - cfg.evap_r_center) / cfg.evap_r_half)**2 + \
              ((Z - cfg.evap_z_center) / cfg.evap_z_half)**2

    if cfg.evap_dipole:
        # 偶极子: 高斯垂直导数 → 上部加热 + 下部冷却
        # Q(z) = -Q0 * (z-zc)/sigma_z^2 * exp(-dist^2)
        # 负号使 Q0<0 时: 上部(z>zc)→加热(Q>0), 下部(z<zc)→冷却(Q<0)
        sz = cfg.evap_dipole_sigma_z
        envelope = np.exp(-((R - cfg.evap_r_center) / cfg.evap_r_half)**2
                          - ((Z - cfg.evap_z_center) / sz)**2)
        z_factor = -(Z - cfg.evap_z_center) / (sz ** 2)
        heating = cfg.evap_cooling_q0 * cfg.evap_dipole_q_factor * z_factor * envelope

        peak_cool = float(np.nanmin(heating))
        peak_heat = float(np.nanmax(heating))
        print(f"[INFO] 层状云偶极子: 峰值冷却={peak_cool:.2e} K/s, 峰值加热={peak_heat:.2e} K/s")
        print(f"       垂直半轴={sz:.1f}km, 缩放因子={cfg.evap_dipole_q_factor:.1f}")
    else:
        # 纯冷却高斯 (原方案)
        heating = cfg.evap_cooling_q0 * np.exp(-dist_sq)
        peak = float(cfg.evap_cooling_q0)
        print(f"[INFO] 层状云蒸发冷却: 峰值={peak:.2e} K/s (≈{peak*86400:.1f} K/day)")

    print(f"       区域: r={cfg.evap_r_center-cfg.evap_r_half:.0f}-{cfg.evap_r_center+cfg.evap_r_half:.0f}km, "
          f"z={cfg.evap_z_center-cfg.evap_z_half:.1f}-{cfg.evap_z_center+cfg.evap_z_half:.1f}km")
    return heating.astype(np.float64)
...
def plot_solution(r_km: np.ndarray, z_km: np.ndarray, psi_rz: np.ndarray, u_rz: np.ndarray, w_rz: np.ndarray, ut_rz: np.ndarray, out_png: Path) -> None:
    ...
    for ax, ff, ttl in zip(axes_flat, fields, titles):
        if ttl == "psi":
            ...
            # U_se: 以出流层(z>8km)最大绝对值为对称colorbar上界
            z_mask = z_km >= 8.0
            ff_upper = ff[z_mask, :]
...
def plot_forcing(r_km: np.ndarray, z_km: np.ndarray, forcing_total: np.ndarray, forcing_thermal: np.ndarray, forcing_momentum: np.ndarray, forcing_evap: np.ndarray, q_2d: np.ndarray, out_png: Path) -> None:
    fig, axes = plt.subplots(2, 3, figsize=(22, 12), constrained_layout=True)
    from matplotlib.colors import TwoSlopeNorm
    fields = [forcing_total, forcing_thermal, forcing_momentum, forcing_evap, q_2d]
    titles = ["Total forcing", "Thermal forcing", "Momentum-source forcing",
              "Evap. heat flux (chi^2 Q)", "Diabatic heating Q (K/s)"]
    axes_flat = axes.flatten()

    for i, (ax, ff, ttl) in enumerate(zip(axes_flat, fields, titles)):
        vmin, vmax = float(np.nanmin(ff)), float(np.nanmax(ff))
        levels_pos = np.linspace(0, vmax, 16)[1:] if vmax > 0 else []
        levels_neg = np.linspace(vmin, 0, 16)[:-1] if vmin < 0 else []
        levels = np.unique(np.concatenate([levels_neg, levels_pos]))
...
def run_pipeline(cfg: PipelineConfig) -> None:
    ...
    # 添加层状云降水蒸发冷却 (椭圆型非绝热加热)
    q_evap = _stratiform_evap_cooling(r_km, z_km, cfg)
    q_mod = q_mod + q_evap
    ...
    plot_forcing(
        ...
        forcing_evap=(fields["chi"]**2) * q_evap,  # 直接显示热力通量 χ²Q, 非梯度
        q_2d=q_mod,
...
def parse_args() -> PipelineConfig:
    ...
    p.add_argument("--no-evap-cooling", action="store_true",
                   help="禁用层状云降水蒸发冷却模块")
    p.add_argument("--evap-cooling-q0", type=float, default=-2.0e-4,
                   help="蒸发冷却峰值强度 (K/s), 负值=冷却, 默认-2e-4")
    ...
    return PipelineConfig(
        ...
        evap_cooling_enabled=(not args.no_evap_cooling),
        evap_cooling_q0=args.evap_cooling_q0,
        evap_r_center=args.evap_r_center,
        evap_z_center=args.evap_z_center,
        evap_r_half=args.evap_r_half,
        evap_z_half=args.evap_z_half,
        evap_dipole=args.evap_dipole,
